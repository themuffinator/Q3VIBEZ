...
static int BG_CountItems( void ) {
	int count = 0;

	while ( bg_itemlist[ count ].classname != NULL ) {
		count++;
	}
...
#ifdef CGAME
	CG_StoreEvent( static_cast<entity_event_t>( newEvent ), eventParm, entityNum );
#endif
